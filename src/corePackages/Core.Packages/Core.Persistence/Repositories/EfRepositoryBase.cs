...
public class EfRepositoryBase<TEntity,TEntityId,TContext>
    ...
    where TEntity : Entity<TEntityId> //generic tipimiz
    where TContext : DbContext
{
    ...
    public async Task<bool> AnyAsync(Expression<Func<TEntity, bool>>? predicate = null, bool withDeleted = false, bool enableTracking = true, CancellationToken cancellationToken = default)
    {
        ...
        if (!enableTracking)   //Eğer takip etme özelliği kapalıysa 
            queryable = queryable.AsNoTracking();
        if(withDeleted)    //Eğer silinmiş verileri de getirmek istiyorsak
            queryable = queryable.IgnoreQueryFilters();   //Query filtrelerini yok say
        if(predicate !=null)   //Bir şart varsa
    ...
    {
        await SetEntityAsDeletedAsync(entity,permanent); //Silinecek mi güncellenecek mi kararını ver
        await Context.SaveChangesAsync();
        return entity;
    }
    ...
    //Entity'nin one-to-one ilişkisi var mı diye kontrol et
